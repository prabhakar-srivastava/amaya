...
async function page({ params }: any) {
    ...
    console.log(docSnap.exists(), 'yes')
    if (docSnap.exists()) {
    ...
    // const data = lashaData?.lasha?.[6]
    const size = lashaData?.lasha?.[6]?.size

    return (
        <div className=''>
            <Section type={TYPE.CONTAIN} sectionStyle={'!py-0  lg:!px-20'}>
                <div className='relative lg:grid grid-cols-2'>
                    <div className=' lg:sticky top-28 h-max'>
                        <ThumbNailCarousal imgList={data?.image} />
                    </div>
                    <div className='px-4 mt-6 lg:mt-0'>
                        <div className='flex gap-2'>
                            {
                                data?.tags?.length > 0 && data?.tags.map((res: string | number | bigint | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<AwaitedReactNode> | null | undefined, index: Key | null | undefined) => {
                        ...
                        <div className='py-4 border-t-2 border-b-2 mt-6'>
                            ...
                            <div className='text-xs font-semibold tracking-wider leading-loose mt-4'>
                                <h1 className=''>Contains : <span className='text-gray-500 font-normal'>{data?.components}</span></h1>
                                <h1 className=''>Fit : <span className='text-gray-500 font-normal'>{data?.fit}</span></h1>
                                <h1 className=''>Composition : <span className='text-gray-500 font-normal'>{data?.composition}</span></h1>
                                <h1 className=''>Care : <span className='text-gray-500 font-normal'>{data?.care}</span></h1>
                            </div>
                        </div>
                        <div className='py-4'>
